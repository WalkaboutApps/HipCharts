...
private let subscriptionProductId = "com.walkabout.hipcharts.defaultSubscription"

class PricingManager {
        
    let subscriptionProduct = CurrentValueSubject<Product?, Never>(nil)
    let purchasedSubscriptionProduct = CurrentValueSubject<Product?, Never>(nil)
    ...
    init() {
        ...
        productFetchTask = loadProductsAndPurchases()
    }
    ...
    @discardableResult
    func loadProductsAndPurchases() -> Task<Void, Error> {
        return Task {
            // load products
            do {
                let products = try await Product.products(for: [subscriptionProductId])
                subscriptionProduct.value = products.first { $0.id == subscriptionProductId }
            } catch {}
    ...
    func hasAccessToPaidFeature(_ feature: PaidFeature, count: Int?) -> Bool {
    ...
    // MARK: - Listen for Product purchase changes
