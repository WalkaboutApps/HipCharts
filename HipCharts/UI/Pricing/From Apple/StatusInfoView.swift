...
//            if let newProduct = pricingManager.subscriptionProduct.value, newProductID == newProduct.id {
//                description = "Your subscription to \(newProduct.displayName)"
//                description += " will renew when your current subscription expires on \(expirationDate.formattedDate())."
//            }
//        } else
        if renewalInfo.willAutoRenew {
            description = "\nNext billing date: \(expirationDate.formattedDate())."
        } else {
            description = "\nYour subscription to \(product.displayName) will end on \(expirationDate.formattedDate())."
